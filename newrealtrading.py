...
class ExecutionClient:
    ...
    def _sanitize_order_params(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """
        Binance USDT-M rules (disederhanakan):
        - ENTRY (MARKET/LIMIT) -> jangan sertakan reduceOnly.
        - STOP/TP close-all: jika closePosition=True -> JANGAN sertakan 'quantity' & 'reduceOnly'.
        - Partial close via STOP/TP (tanpa closePosition): sertakan 'quantity' + reduceOnly=True.
        """
        t = str(params.get("type", "")).upper()
        close_all = bool(params.get("closePosition", False))
        has_qty = params.get("quantity") not in (None, "")

        # ENTRY: hilangkan reduceOnly
        if t in ("MARKET", "LIMIT") and not close_all:
            params.pop("reduceOnly", None)

        # CLOSE-ALL via STOP/TP
        if t in ("STOP_MARKET", "TAKE_PROFIT_MARKET") and close_all:
            params.pop("quantity", None)
            params.pop("reduceOnly", None)

        # Partial close: wajib reduceOnly
        if t in ("STOP_MARKET", "TAKE_PROFIT_MARKET") and not close_all:
            if has_qty:
    ...
    def check_trading_signals(self, df_raw: pd.DataFrame, available_balance: float) -> float:
        ...
        try:
            if self.pending_skip_entries > 0:
                ...
                if getattr(self.pos, "allow_trailing", False):
            ...
            # Basic quality filters (ATR/body) mengikuti flag di config
            atr_enabled = bool(filters_cfg.get('atr', False))
            body_enabled = bool(filters_cfg.get('body', False))
            min_atr_threshold = _to_float(self.config.get('min_atr_pct', DEFAULTS['min_atr_pct']), DEFAULTS['min_atr_pct']) if atr_enabled else 0.0
            max_atr_allowed = _to_float(self.config.get('max_atr_pct', DEFAULTS['max_atr_pct']), DEFAULTS['max_atr_pct']) if atr_enabled else float('inf')
            ...
            atr_ok = True if not atr_enabled else ((last['atr_pct'] >= min_atr_threshold) and (last['atr_pct'] <= max_atr_allowed))
            body_ok = True
            if body_enabled:
                body_val = last.get('body_to_atr', last.get('body_atr'))
                body_ok = (as_float(body_val) <= max_body_over_atr)
            if self.verbose and ((atr_enabled and not atr_ok) or (body_enabled and not body_ok)):
